...
class AggregateManager:
    ...
    def get_top_five(self):
        ...
        ingredient_items = sorted(ingredient_distribution.items(), key=lambda x: x[1], reverse=True)[:5]
        ingredient_names = [item[0] for item in ingredient_items]
        ingredient_counts = [item[1] for item in ingredient_items]
        dish_items = sorted(dish_scores.items(), key=lambda x: x[1], reverse=True)[:5]
